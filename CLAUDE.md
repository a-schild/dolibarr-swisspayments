...
- `createinvoice.php`, `class/swisspayments.class.php` include via `/swisspayments/...`.
...
- `swisspayments_factf` — record of an already-imported ESR/QR bill (dedup), incl. the per-bill creditor `iban` (from the QR) so payment targets the right account when a supplier has several bank accounts — `createDTA()` and the `dtapayments.php` gate prefer it over the supplier's default RIB, falling back to the default RIB only for legacy rows without it (`societe_rib`'s IBAN is still used, separately, to identify the supplier from a scanned QR). Class: `swisspaymentsfactf.class.php`.
...
- **Language**: as of 2026.07.5 the module ships `.lang` files in **DE/FR/IT/EN** under `langs/<locale>/swisspayments.lang`, and the main pages (`createinvoice.php`, `dtapayments.php`, `dtafile.php`, `company_swisspayments.php`) plus the module descriptor (menu titles, permission labels) render user-facing strings through `$langs->trans('SwpXxx')` keys. When you add a user-facing string, add a `Swp`-prefixed key to **all four** lang files rather than hard-coding a literal. A few German literals remain in deep page logic and in stored-content strings (e.g. the `"Lieferantenrechnung"` invoice-line description) — match the surrounding style of the file you edit.
- **Dolibarr version compatibility (15.x → 24.x)**: the module must run on the whole range, so do **not** call APIs added mid-range (`User::hasRight()`, `getDolGlobalString()` — Dolibarr 16+) directly in page/class code. Route them through the runtime shims in `lib/swisspayments.lib.php`: `swisspayments_user_has_right($user, $mod, $l1, $l2='')`, `swisspayments_user_socid($user)` (prefer `socid` over the deprecated `societe_id`), `swisspayments_conf_string($key, $default='')`, `swisspayments_mod_enabled($module)`. Each prefers the modern API when it exists at runtime and falls back to the legacy accessor (`$user->rights->…`, `$conf->global->…`, `$conf->{module}->enabled`) otherwise. **Exception:** the module descriptor's menu `enabled`/`perms` strings (`'$conf->swisspayments->enabled'`, `'$user->rights->swisspayments->invoices->create'`) are `dol_eval`'d by core and must stay in the legacy `$conf->…->enabled` / `$user->rights->…` form — core still evaluates those on every version, and `hasRight()` in a perms string would fatal on Dolibarr 15. The descriptor declares the tested range via `need_dolibarr_version`/`max_dolibarr_version`.
...
- **Release process**: bump `const VERSION`, add a dated `CHANGELOG.md` section, land the work on `main`, then push a tag named exactly for the version (bare `YYYY.MM.n`, no `v` prefix — e.g. `2026.07.6`). **Pushing the tag is the trigger**: `.github/workflows/release.yml` auto-builds the module zip (`module_swisspayments-<version>.zip`) and **publishes** a GitHub Release as `github-actions[bot]` with minimal auto-generated "What's Changed" notes. So after tagging, the release already exists — do **not** create a second one (`gh release create` would make a duplicate on the same tag). To improve the notes, edit the existing release in place (`gh release edit <tag> --notes-file …`, or PATCH it by id), which keeps the built zip asset. If a duplicate release ever exists on one tag, resolve them by release **id** (`gh api repos/<owner>/<repo>/releases`) — deleting/editing by tag is ambiguous when two share it.
- **Open work / known issues live in [`todo.md`](todo.md)** — tracked items that aren't quick code fixes. The batch-file IDOR, multi-entity isolation, the deprecated legacy page and the missing `.lang` files were all closed in 2026.07.5. Every module table now carries an `entity` column (set to `$conf->entity` on insert, filtered `entity IN (getEntity(...))` on read) and `swisspayments_payh` also has an `fk_user_author` owner column (enforced in `dtafile.php`). Schema changes are defined once in `swisspayments_db_columns()` (in `lib/swisspayments.lib.php`) and applied by the idempotent `swisspayments_migrate_tables()` — called both from `modswisspayments::init()` (on activation) and from `swisspayments_check_db_version()`, a runtime guard invoked on every module entry page that auto-migrates after a plain file/zip update (tracked via the `SWISSPAYMENTS_DB_VERSION` constant) so no disable/re-enable is needed. When you add a column, add it to `swisspayments_db_columns()` **and** bump `VERSION`. Consult todo.md before starting non-trivial work, and keep it in sync when you close or add an item.
